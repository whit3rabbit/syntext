...
### Fixed
- **A git checkout nested inside the repo is no longer indexed as a second copy of itself.** `git worktree add worktrees/foo` (or a submodule, or a plain clone) that is not gitignored was walked like ordinary content, so every file appeared twice and every search returned both. The duplication was also incoherent with freshness rather than merely redundant: git refuses to descend into a directory holding its own `.git`, so `git status --porcelain -uall` reports the whole subtree as a single record `worktrees/foo/`, with a trailing slash. The walk indexed N files that detection could never report per-file, and that lone directory record could never be applied, so it stayed in the change set forever. Reproduced on a two-file fixture: before, `st index` reported 35 documents, a search returned both `marker.rs` and `wt-x/marker.rs`, and `st status` printed `Behind: 1` immediately after a clean index and kept printing it. After: 32 documents, one match, and `Behind: 0`. That permanent staleness signal is what re-spawned the detached `st update --quiet` catch-up on every search. Two independent changes: `enumerate_files` (`src/index/walk.rs`) prunes any directory below the root that holds a `.git`, and `parse_status_z` (`src/index/porcelain.rs`) drops records whose path ends in `/`, which costs no syscall because git never emits a trailing slash for a file. `apply_changed_paths` also skips directories as a backstop. The repository's own `.git` is untouched, since there is no `.git/.git`, so `.git/hooks/*.sample` is still indexed and document counts that depend on it are unchanged.

### Added
- **`syntext::changes` subsystem** (`src/changes/`): persistent change catalogue with BLAKE3 content fingerprinting, racily-clean mtime settlement (2 s margin), and batch ingestion into the index. `ContentDigest` provides domain-separated streaming BLAKE3 hashes. `FileFingerprint` and `StatObservation` track per-file state across restarts. `ChangeBatch` tags each path as `Added`, `Modified`, `Touched`, `Deleted`, `Unchanged`, or `Uncertain`. `Catalogue` persists fingerprints and consumer acknowledgements in a checksummed binary file (`catalogue.idx`, xxHash64 validated). `Index::apply_change_batch` (`src/index/batch_apply.rs`) accepts pre-loaded in-memory buffers so observed bytes are not re-read from disk; the underlying `commit_inner_with_preloaded` path is shared with the normal commit flow.
- **`st index --index-nested`** opts back in to indexing nested checkouts, for anyone who was relying on submodule content being searchable. Note what you get: git will not report those files individually, so they cannot be refreshed incrementally and go stale until the next full rebuild. New `Config::index_nested_checkouts` field, default `false`. `st index` prints one line per pruned checkout (`st: skipping nested worktree wt-x (use --index-nested to include)`, capped at three plus a count), at debug level, which `st index` shows by default and `--quiet` suppresses. The internal `rebuild_if_stale` rebuild runs at warn level and stays silent, so nothing new reaches the search path.
- **`st status` reports which checkout the index belongs to**, so an agent working in a worktree can tell whose tree it is about to edit: `Worktree:  wt-x (linked)` and `Branch:    side` in the text output, and `worktree_kind`, `worktree_name`, `branch`, `detached_head` in `--json`. Kinds are `main`, `linked`, `submodule`, `nested-clone`. New module `src/git_checkout.rs`, which reads `.git` and `<gitdir>/HEAD` directly and never spawns `git`: `detect_changed_files` budgets exactly one `git status` spawn per detection and two tests in `tests/integration/cli.rs` assert that count. Both reads are `O_NOFOLLOW` and capped at 4 KiB, and names and branches carrying control characters are rejected rather than scrubbed, so a hostile repo cannot inject terminal escapes into `st status`. Deliberately not added to the search `--json` summary, which is rg-compatible and compared by the differential oracle.

### Changed
- `WalkSkips` (`syntext::index::walk`) gains a `nested_checkouts` field and loses `Copy`. `Config` gains `index_nested_checkouts`. It is not `#[non_exhaustive]`, so an exhaustive struct literal in downstream code needs the new field (use `..Config::default()`).
- `st status` moved out of `src/cli/manage.rs` into `src/cli/status.rs` to stay under the 400-line quality gate. Behaviour is unchanged by the move.
...
### Changed
- **Git change detection is one spawn instead of three.** `detect_changed_files` ran `git diff HEAD`, `git diff --cached`, and `git ls-files --others` back to back on every search, and the path was pure subprocess overhead. It now runs a single `git status --porcelain=v1 -z -uall --no-renames --no-branch` and parses the records (new `src/index/porcelain.rs`). `diff --cached` was redundant on top of that: if the index differs from HEAD then either the worktree differs from the index or it matches the index and differs from HEAD, so `status` reports the path either way, and syntext only ever indexes worktree bytes. Measured on a 2000-file repo, `bench_freshness/detect_no_changes_2000_files` went from 35.8 ms to 15.5 ms and `detect_one_changed_file_2000_files` from 33.9 ms to 15.2 ms. `update_from_git_bounded_2000_files`, which is detection plus the overlay apply, went from 36.6 ms to 17.0 ms. One spawn also matters most on a loaded box, where per-spawn latency is what blows up. Every flag is load-bearing and documented on the `STATUS_ARGS` constant: `-uall` in particular, because the default mode collapses an untracked directory to `newdir/` and the update path would then notify a directory and miss every file inside it. A deadline-torn final record is now dropped rather than counted, so the files-behind estimate after a budget kill can no longer include a path prefix that does not exist. The shim-counting tests in `tests/integration/cli.rs` now expect one logged git call per detection instead of three.
- **`st update` now persists uncommitted working-tree drift.** Previously it applied the drift to its own in-memory overlay and exited. Unless the overlay crossed its 50%-of-base cap (which forces a full rebuild), nothing reached disk, so the next `st search` was a fresh process with an empty overlay: it re-detected the same files, searched stale, and spawned another catch-up child that did the same thing again. New `Index::flush_overlay` (`src/index/flush.rs`) writes the committed overlay out through the same `delta_apply` path the committed-HEAD delta uses, differing only in that `base_commit` is carried forward untouched. Advancing it would make `rebuild_if_stale` (which only fires on `base_commit != HEAD`) skip the real commit when it lands. The detached catch-up child needed no argv change: it already ran `st update --quiet`, and that is now durable. `st update --flush` remains accepted and does nothing. A change set past the overlay's 50%-of-base cap reports "nothing flushed" rather than an error: `update_from_git` answers that case with an inline full rebuild, which is durable on its own, so surfacing it as a flush failure would tell the user to run `st index` immediately after one had just run. Those paths get no anchor, though, so they keep being re-detected until they are committed.
- **New working-tree anchor sidecar** (`worktree-<uuid>.idx`, `src/index/worktree_anchor.rs` + `worktree_codec.rs`, recorded in `Manifest::worktree_anchor_file`). `git diff HEAD` reports an uncommitted file forever, so without this every search after a flush would re-read and re-apply content the index already holds. The sidecar records size and mtime per flushed path (plus `Absent` for deletions and `Present` for excluded binary/oversized paths, so a deleted tracked file does not cost a `notify_delete` per search), and `update_from_git` and `st status` drop paths that have not moved since. It fails OPEN, unlike the delete-set sidecar: losing it costs re-applies, never correctness, because `compute_delete_set` is idempotent. A path is anchored only when its mtime is at least 2 seconds older than the commit's content read, which is git's racily-clean rule; a racy path is left out and re-applied once. Over 50,000 entries no anchor is written at all. Compaction carries the filename forward, and `Manifest::gc_orphan_segments` sweeps stale generations. `Manifest::overlay_gen` now counts durable flushes instead of being reserved.
- **Injected agent guidance rewritten** (`st init`, `st agent install`). The old text told the agent to run `st update` after every edit, which cost a tool call per edit for something search already does on its own: bounded update-on-search refreshes from git on every call. The new text says not to, and to run `st update` only when `st` prints "files behind". It also tells the agent to bound its own output (`--max-results N`, `-l`, `-c`, `-m N`, `-C N`, `-g`, `-t`) and to treat a returned line with context as already-read evidence rather than following up with a file read. The awareness file (`SYNTEXT.md`) and the marker-guarded rules block now share one literal, so the two can no longer drift apart. Existing installs are not rewritten in place: `st agent uninstall <agent>` then `st agent install <agent>` picks up the new text.
...
### Docs
- **README rewritten against the code.** The tagline claimed "up to 20x+ faster than ripgrep" while the table under it topped out at 3.5x, and it sat inside a `<pre>` block so the bold rendered as literal asterisks. It now states the measured 1.2x to 3.5x range, the 18x best case (`raw_spin_lock` on the Linux kernel, 133 ms against 2,443 ms), and the caveats: averages over token-aligned queries, index build excluded, the TypeScript row suspect because both its queries undercounted. Bounded update-on-search, which the README had only mentioned under Known limitations, has its own subsection, and Usage no longer tells the reader to run `st update` after every edit. The claim that hooks never run `st update` is scoped to the shell-rewrite hooks, since the git-hooks integration does. Added a trust paragraph linking `docs/SECURITY.md`, the Swift bindings, `-S`, `-w`, `-e`, `-f`, `-C`, `-T`, `-o`, `-m`, `--vimgrep`, `--files`, `--files-without-match`, `st status --json`, `st verify`, `st index --recalibrate`, `st init --fsmonitor`, `SYNTEXT_NO_AUTO_UPDATE`, `SYNTEXT_QUIET_FALLBACK`, and a Docs list covering every file under `docs/`. Install examples pin the latest published release.
- `install.sh` default `SYNTEXT_VERSION` bumped from 2.1.0 to 2.2.0. The v2.2.0 release missed the post-release installer bump in `docs/RELEASE.md`.
