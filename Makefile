...
# Output: target/<target>/debian/syntext_<version>_<arch>.deb

deb-amd64: build-linux-amd64
	cargo deb --no-build --target x86_64-unknown-linux-gnu
	cp target/x86_64-unknown-linux-gnu/debian/syntext_$(VERSION)_amd64.deb \
	   $(DIST_DIR)/

deb-arm64: build-linux-arm64
	cargo deb --no-build --target aarch64-unknown-linux-gnu
	cp target/aarch64-unknown-linux-gnu/debian/syntext_$(VERSION)_arm64.deb \
	   $(DIST_DIR)/
