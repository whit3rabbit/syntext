...
- **SC-004**: Search results are identical to ripgrep for all supported pattern types (correctness).
  - **Ripgrep version**: `ripgrep 15.1.0` (specifically the version pinned in `tests/integration/correctness.rs` via a `rg --version` assertion). The test suite MUST fail if the installed `rg` version differs from the pinned version.
