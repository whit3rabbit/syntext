...
use ripline::tokenizer::{build_all, build_covering, gram_hash, MIN_GRAM_LEN};
...
fn input_exactly_max_gram_len() {
    ...
    let input = vec![b'a'; ripline::tokenizer::MAX_GRAM_LEN];
...
fn input_just_over_max_gram_len() {
    ...
    let input = vec![b'a'; ripline::tokenizer::MAX_GRAM_LEN + 1];
    let all = build_all(&input);
    assert!(
        all.is_empty(),
