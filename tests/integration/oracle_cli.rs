...
use oracle_helpers::{run_differential, run_differential_with_tier_c, generate_corpus, generate_query, generate_flags};
...
fn golden_smoke_anchored_literal() {
    let corpus = vec![
        ("src/main.rs".to_string(), b"fn _parse_query_() {}\n".to_vec()),
        ("src/lib.rs".to_string(), b"fn reparse_query() {}\n".to_vec()),
    ];
...
fn golden_smoke_crlf_line_endings() {
    let corpus = vec![
        ("src/main.rs".to_string(), b"fn foo() {}\r\nfn bar() {}\r\n".to_vec()),
    ];
...
fn golden_smoke_cr_only_line_endings() {
    let corpus = vec![
        ("src/main.rs".to_string(), b"fn foo() {}\rfn bar() {}\r".to_vec()),
    ];
    run_differential(&corpus, "foo", &[]).unwrap();
}
...
fn golden_smoke_utf16_le() {
    ...
    let corpus = vec![
        ("src/main.rs".to_string(), content_u16.to_vec()),
    ];
...
fn golden_smoke_invalid_utf8_in_content() {
    let corpus = vec![
        ("src/main.rs".to_string(), vec![b'f', b'n', b' ', b'p', b'a', b'r', b's', b'e', 0xFF, 0xFE, b'\n']),
    ];
    run_differential(&corpus, "parse", &[]).unwrap();
}
...
fn golden_smoke_alternation() {
    let corpus = vec![
        ("src/main.rs".to_string(), b"fn parse() {}\nfn query() {}\n".to_vec()),
    ];
...
fn golden_smoke_word_regexp_alternation() {
    let corpus = vec![
        ("src/main.rs".to_string(), b"foo bar foobar barfoo\n".to_vec()),
    ];
...
fn golden_smoke_only_matching_context() {
    let corpus = vec![
        ("src/main.rs".to_string(), b"before\nfn parse_query() {}\nafter\n".to_vec()),
    ];
...
fn golden_smoke_word_regexp() {
    let corpus = vec![
        ("src/main.rs".to_string(), b"parse parse_query reparse\n".to_vec()),
    ];
...
fn test_regression_fixtures() {
    ...
    for entry in entries {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("json") {
            ...
            let corpus: Vec<(String, Vec<u8>)> = fixture.corpus.iter().map(|fe| {
