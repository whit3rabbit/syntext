...
- **Description**: rendered output now keeps a CRLF line's trailing `\r` (byte-identical to rg), but `st` *matches* against the `\r`-stripped line — `rg --crlf` semantics (a deliberate design choice: see the `json_submatches` comment on `-x` submatch extraction). rg's default treats the `\r` as ordinary line content. Differences: patterns that would match the `\r` byte itself get different submatch spans, and — for line-anchored matching (`-x`, or a `$` in the pattern) — WHICH lines match can differ (st's `parse` fully matches `parse\r`; rg's does not without `--crlf`).
- **Harness Action**: stdin comparisons are byte-exact (no `\r` normalization). The proptest query pool is token-aligned (no `\r`-matching patterns); `-x` is dropped from the flag set when the stream contains `\r` (the anchor-semantics difference above).

### 16. Binary Files: never-indexed files stay invisible; late-NUL chunks differ

- **Description**: stdin/verifier-level binary handling now matches rg byte-for-byte for single-read-chunk streams (a NUL replaces all line output with `binary file matches (found "\0" byte around offset N)`, exit 0 when a match exists; `-c`/`-l`/`-q`/`--json` keep normal output, with NUL treated as a line terminator for counts/line numbers; no match exits 1 silently). Two divergences remain: (1) structural — binary *repo files* are never indexed, so indexed search cannot see matches in them (misses only, never false positives); (2) when the first NUL lands beyond rg's first read chunk (~8KB), rg prints the matches preceding that chunk normally before the notice, while `st` always prints only the notice — rg's chunk size is a reader-internal `st` does not emulate.
- **Harness Action**: The stdin proptest excludes NUL-containing corpus files (chunk-boundary behavior is not reproducible); `stdin_golden_binary_stream_notice_parity` covers the single-chunk binary stream byte-exactly. File-based corpora keep the match-after-NUL shape (`golden_smoke_binary_null_at_8191`), where rg's implicit-binary quit-agreement makes both engines report nothing; corpus files with matches before a NUL are not generated.
