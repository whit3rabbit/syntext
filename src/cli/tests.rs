    ...
    fn line_number_and_filename_aliases_match_ripgrep() {
        let cli = Cli::try_parse_from(["st", "-N", "-n", "-I", "-H", "pattern"])
            .expect("parse failed");
        assert!(cli.line_number, "last line-number flag should win");
        assert!(!cli.no_line_number, "overridden no-line-number should be cleared");
    ...
    fn line_regexp_overrides_word_regexp() {
        ...
        assert!(!cli.word_regexp, "overridden boundary mode should be cleared");
    }
    ...
    fn max_count_applies_per_file() {
        ...
        let results = super::search::run_search(&index, &args).unwrap();
        assert_eq!(results.len(), 2, "expected one match from each file");
        assert_eq!(
            results.iter().filter(|m| m.path == std::path::Path::new("one.rs")).count(),
            1
        );
        assert_eq!(
            results.iter().filter(|m| m.path == std::path::Path::new("two.rs")).count(),
            1
        );
    }
