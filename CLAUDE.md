...
- **Load-sensitive tests: never race a wall clock against a real subprocess.** On macOS, `syspolicyd` assesses every freshly linked unsigned binary, so a loop of Rust relinks can pin it at ~470% CPU for hours. Measured spawn-to-first-byte for `/bin/sh` in that state: 3.8s, 5.9s, over 10s, against 5-9ms idle. Any test that assumes a spawned process meets a fixed deadline will flake, and raising the constant only moves the threshold. Two working patterns, both in `src/index/freshness_tests.rs`: **calibrate** the deadline from a measured warm-up spawn (`calibrated_kill_deadline_ms`, 10x clamped to 200ms..30s), and have shims `exec 1>&-` before stalling (`STALL_AFTER_CLOSING_STDOUT`) so `read_to_end` gets EOF at the kill instead of waiting out a `sleep` grandchild that inherited the pipe. Do NOT wall-clock a spawned `st` to prove in-process behavior: exec latency alone swamps sub-second assertions. Assert such timing in-process instead (`cli::open_retry`'s `open_for_search_waits_out_a_real_held_lock`). Related: `SYNTEXT_NO_ASYNC_UPDATE=1` isolates a test from the detached catch-up writer, and `SYNTEXT_AUTO_UPDATE_BUDGET_MS` from git-detection timing.
...
- **`LockConflict` does not always mean contention.** `helpers::classify_try_lock` (`src/index/helpers.rs`) maps any non-`WouldBlock` `flock(2)` failure onto it too (`ENOLCK` under the nightly's process churn is the observed one on macOS; `EINTR` is retried in place first). So a `LockConflict` on a private index dir no other process ever opened is a kernel resource failure, not a harness bug -- the errno is in the `log::warn!`. Every `Index::build` / `Index::open` / `commit_batch` call in the oracle harnesses goes through a bounded exponential-backoff retry for this reason.
...
- **Stdin filter mode** (rg parity): `cmd | st 'pat'` and `st 'pat' -` search the stream in-memory before the index is opened (`src/cli/stdin_search.rs`), so they work with no `.syntext`. Implicit-stdin detection accepts only a FIFO/regular-file stdin, NOT merely "not a tty". Agent shells attach `/dev/null` or a socket to stdin, and reading those would silently search an empty stream. **Classify the descriptor, never the path**: `fd_is_searchable` `fstat`s a dup of fd 0. Stating `/dev/stdin` goes through macOS's `fdesc` lookup, which transiently returns `EBADF` under process churn. Measured: 0.19% of invocations on a loaded box, 0% for `fstat`. The fail-safe arm then searched the repo instead of the stream. `-` mixed with real paths searches BOTH (merged output, stdin run ordered by argv position; exceptions: `-v` mixed exits 2, and mixed + missing index errors because stdin is already consumed). `-v` inverts per-line on streams. Binary (NUL) streams follow rg's notice semantics; rendered lines keep CRLF `\r` (see `tests/oracle/DIVERGENCES.md` #14-#16 for the remaining documented divergences).
- **Search waits briefly for the index lock.** `cmd_search` opens via `cli::open_retry::open_for_search`, which retries ONLY `IndexError::LockConflict` on a fixed schedule (20, 40, 80, 160, 200ms, so 500ms total) and then fails with the same error and exit 2. `st` creates this contention itself: a budget-overrunning search spawns a detached `st update --quiet` whose exclusive window can swallow the next search's open (measured 34/100 spurious exit-2s at a 150ms transient lock, 0/100 after). Every other error, `CorruptIndex` and `IndexNotFound` included, still returns on the first attempt, so corruption is never masked behind a wait, and an uncontended open never sleeps.
...
- **Overlay is in-memory only.** `commit_batch` persists nothing to disk (`open.rs` loads `OverlayView::empty()`); only `build::build_index`, compaction, and an explicit flush write durable segments. An in-search overlay-apply is invisible to a later `st search` process, so cross-process freshness comes from `Index::flush_overlay` (`src/index/flush.rs`), the durable delta path below, or a full rebuild, never a bare overlay-apply.
- **`st update` always persists**, including uncommitted working-tree drift. `flush_overlay` reuses `delta_apply` with `FlushAnchor::KeepHead`, which carries `base_commit` forward instead of advancing it: drift belongs to no commit, and advancing would make `rebuild_if_stale` (which only fires on `base_commit != HEAD`) skip the real commit when it lands. `st update --flush` is accepted and does nothing.
