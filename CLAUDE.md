...
- See [`docs/KNOWN_ISSUES.md`](docs/KNOWN_ISSUES.md) for open design issues, notably the gram index candidate narrowing bypass due to weight table boundary calibration.
...
- **Tokenizer**: sparse n-grams with pre-trained byte-pair frequency weight table. Lowercase normalization at index/query time. Weight table lives in `src/tokenizer/weights.rs`.
- **Query router**: literal (memchr::memmem) / indexed regex (HIR decomposition) / full scan. Path filter always first.
